...
class MPU6050(object):
	...
	I2CADDR = 0x68	   # This is the address value read via the i2cdetect command

	def __init__(self, fs_scale=FS_250, afs_scale=AFS_2g,address=I2CADDR , i2c=None, **kwargs):
		# Create I2C device.
		if i2c is None:
		...
		self._device.write8(MPU6050.PWR_MGMT_1, 0)
		# Set the gryo resolution
		self._device.write8( MPU6050.FS_SEL, self.fs_scale << 3)
		# Set the accelerometer resolution
		self._device.write8( MPU6050.AFS_SEL, self.afs_scale << 3)
	...
	def read_raw_data(self):
		...
		'''
		self.raw_gyro_data = self._device.readList( MPU6050.GYRO_START_BLOCK, 6)
		self.raw_accel_data = self._device.readList( MPU6050.ACCEL_START_BLOCK, 6)
		self.raw_temp_data = self._device.readList( MPU6050.TEMP_START_BLOCK, 2)
		
		self.gyro_raw_x = self.twos_compliment(self.raw_gyro_data[MPU6050.GYRO_XOUT_H], self.raw_gyro_data[MPU6050.GYRO_XOUT_L])
		self.gyro_raw_y = self.twos_compliment(self.raw_gyro_data[MPU6050.GYRO_YOUT_H], self.raw_gyro_data[MPU6050.GYRO_YOUT_L])
		self.gyro_raw_z = self.twos_compliment(self.raw_gyro_data[MPU6050.GYRO_ZOUT_H], self.raw_gyro_data[MPU6050.GYRO_ZOUT_L])
		
		self.accel_raw_x = self.twos_compliment(self.raw_accel_data[MPU6050.ACCEL_XOUT_H], self.raw_accel_data[MPU6050.ACCEL_XOUT_L])
		self.accel_raw_y = self.twos_compliment(self.raw_accel_data[MPU6050.ACCEL_YOUT_H], self.raw_accel_data[MPU6050.ACCEL_YOUT_L])
		self.accel_raw_z = self.twos_compliment(self.raw_accel_data[MPU6050.ACCEL_ZOUT_H], self.raw_accel_data[MPU6050.ACCEL_ZOUT_L])
		
		self.raw_temp = self.twos_compliment(self.raw_temp_data[MPU6050.TEMP_OUT_H], self.raw_temp_data[MPU6050.TEMP_OUT_L])
