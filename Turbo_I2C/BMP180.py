...
class BMP180(object):
	...
	def __init__(self, mode=BMP180.STANDARD, address=BMP180.I2CADDR, i2c=None, **kwargs):
		self._logger = logging.getLogger('Turbo_I2C.BMP180')
		# Check that mode is valid.
		if mode not in [BMP180.ULTRALOWPOWER, BMP180.STANDARD, BMP180.HIGHRES, BMP180.ULTRAHIGHRES]:
			raise ValueError('Unexpected mode value {0}.  Set mode to one of BMP180.ULTRALOWPOWER, BMP180.STANDARD, BMP180.HIGHRES, or BMP180.ULTRAHIGHRES'.format(mode))
		self._mode = mode
		# Create I2C device.
		if i2c is None:
	...
	def _load_calibration(self):
		self.cal_AC1 = self._device.readS16BE(BMP180.CAL_AC1)   # INT16
		self.cal_AC2 = self._device.readS16BE(BMP180.CAL_AC2)   # INT16
		self.cal_AC3 = self._device.readS16BE(BMP180.CAL_AC3)   # INT16
		self.cal_AC4 = self._device.readU16BE(BMP180.CAL_AC4)   # UINT16
		self.cal_AC5 = self._device.readU16BE(BMP180.CAL_AC5)   # UINT16
		self.cal_AC6 = self._device.readU16BE(BMP180.CAL_AC6)   # UINT16
		self.cal_B1 = self._device.readS16BE(BMP180.CAL_B1)     # INT16
		self.cal_B2 = self._device.readS16BE(BMP180.CAL_B2)     # INT16
		self.cal_MB = self._device.readS16BE(BMP180.CAL_MB)     # INT16
		self.cal_MC = self._device.readS16BE(BMP180.CAL_MC)     # INT16
		self.cal_MD = self._device.readS16BE(BMP180.CAL_MD)     # INT16
	...
	def read_raw_temp(self):
		...
		self._device.write8(BMP180.CONTROL, BMP180.READTEMPCMD)
		time.sleep(0.005)  # Wait 5ms
		raw = self._device.readU16BE(BMP180.TEMPDATA)
		self._logger.debug('Raw temp 0x{0:X} ({1})'.format(raw & 0xFFFF, raw))
		return raw

	def read_raw_pressure(self):
		"""Reads the raw (uncompensated) pressure level from the sensor."""
		self._device.write8(BMP180.CONTROL, BMP180.READPRESSURECMD + (self._mode << 6))
		if self._mode == BMP180.ULTRALOWPOWER:
			...
			time.sleep(0.008)
		msb = self._device.readU8(BMP180.PRESSUREDATA)
		lsb = self._device.readU8(BMP180.PRESSUREDATA+1)
		xlsb = self._device.readU8(BMP180.PRESSUREDATA+2)
